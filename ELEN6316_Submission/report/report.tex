...
\author{Yi-Hsiang Wei~and~Zijian Shang,~\IEEEmembership{Students,}
        \IEEEmembership{Department of Electrical Engineering, Columbia University}}

\markboth{ELEN E6316 Analog-Digital Interface, Spring 2026}%
...
Across TT, SS, and FF process corners, the design achieves a minimum SFDR of
48.2\,dB across the tested Nyquist-band input frequencies, meeting the 48\,dB
SFDR requirement. The design also achieves DNL\,$<$\,0.18\,LSB and
INL\,$<$\,0.24\,LSB, well within the $\pm$1/$\pm$2\,LSB limits.
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=0.7\linewidth]{figures/top_level.png}
...
\hline
 & W & L & Multiplier \\
\hline
M1 (Current Source) & 1.183\,\textmu m & 500\,nm & weight \\
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{figures/Trim.png}
...
\hline
 & W & L & Multiplier \\
\hline
M1 (Trim Source) & 220\,nm & 20\,\textmu m & weight \\
M2 (Gate P)      & 4.5\,\textmu m & 180\,nm & 2 \\
M3 (Gate N)      & 4.5\,\textmu m & 180\,nm & 1 \\
M4 & 10\,\textmu m & 180\, nm & 1 \\
M5 & 10 \textmu m & 180\, nm & 1 \\
\hline
\end{tabular}
\end{table}
...
\begin{figure}[H]
    ...
    \caption{Single-bit retimer cell (NAND-latch based). Msize's, the sizing parameters, are labelled on each components}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{figures/NAND.png}
    \caption{NAND gate schematic.}
    \label{fig:NAND}
\end{figure}
NAND gate near output are using larger sizing to minimize delay and improve drivability. 
\begin{table}[h]
\centering
\caption{Input Retimer NAND and Inverter Sizing}
\begin{tabular}{lccc}
\hline
 & W & L & Multiplier \\
\hline
...
Dynamic performance is evaluated using six coherent input tones with M = 2048 samples and
J = 17, 127, 251, 503, 751, and 997, where fin = Jfs/M. The exported output spectrum is
post-processed with zero-order-hold sinc correction before calculating SFDR, SNDR, and ENOB.
DC bins and the fundamental ±2 bins are excluded when identifying the largest spur, while the
fundamental power is integrated over ±2 bins for SNDR and SFDR calculation.
All simulations run at $T = 27^\circ$C across TT, SS, and FF corners.

\subsection{Output Impedance}

The output impedance is characterized by an AC frequency sweep at each of the
$2^8=256$ input codes, producing $R_\text{out}$ per code.
The low-frequency (DC) output resistance is extracted and shown in
Fig.~\ref{fig:rout_vs_code} which plots $R_\text{out,diff}$ and $R_\text{out,CM}$
at low frequency against the 8-bit binary input code.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{figures/Fig_Rout_vs_code.png}
    \caption{Low-frequency $R_\text{out,diff}$ (top) and $R_\text{out,CM}$ (bottom)
             vs.\ 8-bit binary input code. Dashed lines mark the 100\,$\Omega$
             and 25\,$\Omega$ specifications.}
    \label{fig:rout_vs_code}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{figures/Fig_Zout_vs_freq.png}
    \caption{$|Z_\text{out}(f)|$ vs.\ frequency for codes 0, 128, and 255 —
             differential (top) and common-mode (bottom).}
    \label{fig:zout_vs_freq}
\end{figure}

The differential output resistance varies only from 98.76\,$\Omega$ to 99.84\,$\Omega$
across all 256 codes ($<$1.1\% variation), and the common-mode resistance from
24.69\,$\Omega$ to 24.96\,$\Omega$ ($<$1.1\% variation).
Both are within 1.3\% of their respective 100\,$\Omega$ and 25\,$\Omega$ targets,
confirming the specifications are met across the full code range.
The tiny code dependence arises from the finite output impedance $r_o$ of the
NMOS current sources appearing in parallel with the 50\,$\Omega$ load resistors;
since $r_o \gg 50\,\Omega$ for these devices, the effect is negligible.
...
\subsection{Retimer Timing Verification}

The input retimer re-times all 8 data bits to the rising edge of CLK\_T,
ensuring simultaneous switching of all current cells.
Fig.~\ref{fig:ret_timing} shows the input data $D\langle0\rangle$, the retimed
output $D_\text{ret}\langle0\rangle$, and both clock phases over a 40\,ns window
(10 clock cycles at 250\,MHz).
Fig.~\ref{fig:ret_delay} zooms in to a single transition to annotate the
CLK\_T-to-$D_\text{ret}$ propagation delay.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{figures/Fig_Retimer_timing.png}
    \caption{Retimer waveforms: input $D\langle0\rangle$, retimed output
             $D_\text{ret}\langle0\rangle$, CLK\_T, and CLK\_C over 40\,ns.}
    \label{fig:ret_timing}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{figures/Fig_Retimer_delay.png}
    \caption{Zoomed view showing the CLK\_T-to-$D_\text{ret}$ propagation delay
             annotated at the 50\% crossing.}
    \label{fig:ret_delay}
\end{figure}

\begin{table}[h]
  \renewcommand{\arraystretch}{1.2}
  \caption{Retimer Timing Metrics (TT Corner, 250\,MHz)}
  \label{tab:retimer}
  \centering
  \begin{tabular}{lc}
    \toprule
    Parameter & Value \\
    \midrule
    CLK\_T $\to$ $D_\text{ret}$ rising-edge delay  & 0.616\,ns \\
    CLK\_T $\to$ $D_\text{ret}$ falling-edge delay & 0.644\,ns \\
    Average propagation delay $t_{pd}$             & 0.627\,ns \\
    $t_{pd}$ as fraction of clock period           & 15.7\,\% \\
    \midrule
    $D_\text{ret}$ rise slew (10\%–90\%)           & 7.63\,V/ns \\
    $D_\text{ret}$ fall slew (90\%–10\%)           & 14.90\,V/ns \\
    \bottomrule
  \end{tabular}
\end{table}

The average propagation delay of 0.627\,ns is 15.7\% of the 4\,ns clock period,
leaving ample setup margin for the current-cell switches.
The falling edge is faster than the rising edge (14.9\,V/ns vs.\ 7.6\,V/ns),
consistent with the stronger NMOS pull-down path in the output buffer inverters.
Both edges are fully settled well within one clock cycle, confirming that
$D_\text{ret}$ presents a clean, synchronous signal to the DAC current-cell switches.

\subsection{Static Linearity — INL and DNL}

Table.~\ref{tab:static} shows INL and DNL across all three process corners.
...
Per-bit current deviation as the 6-bit trim code
is swept from 0 to 63 (reference = trim\,32).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{figures/Fig_Tuning_Range.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{figures/Fig_Spectrum_low.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{figures/Fig_Spectrum_high.png}
    \caption{Output spectrum at $f_\text{in} = 121.7$\,MHz (TT).
             SFDR\,=\,55.7\,dB, SNDR\,=\,48.7\,dB.}
    \label{fig:spec_high}
\end{figure}

SFDR and SNDR vs.\ Input Frequency:
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{figures/Fig_SFDR_vs_freq.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{figures/Fig_SNDR_vs_freq.png}
...
\begin{table}[!h]
  ...
  \begin{tabular}{lccc}
    ...
    \midrule
    $R_\text{out,diff}$ range ($\Omega$)           & \multicolumn{3}{c}{98.76–99.84} \\
    $R_\text{out,CM}$ range ($\Omega$)            & \multicolumn{3}{c}{24.69–24.96} \\
    Trim tuning range (\%)                        & \multicolumn{3}{c}{$-14.8$ to $+14.3$} \\
    \midrule
    Power — Retimer (mW) @ 2.1\,MHz              & \multicolumn{3}{c}{20.9} \\
    Power — Retimer (mW) @ 121.7\,MHz            & \multicolumn{3}{c}{30.6} \\
    Power — DAC + Load (mW)                      & \multicolumn{3}{c}{$\approx$30.0} \\
    Power — Bias Generator (mW)                  & \multicolumn{3}{c}{0.18} \\
    Total Power (mW) @ 2.1\,MHz         & \multicolumn{3}{c}{51.0} \\
    Total Power (mW) @ 121.7\,MHz       & \multicolumn{3}{c}{60.8} \\
    \bottomrule
  \end{tabular}
\end{table}
...
Power is measured by time-averaging the instantaneous supply power from
10\,ns to 990\,ns (one full $\mu$s window) in transient simulation at three
representative input frequencies in the TT corner.
Table.~\ref{tab:power} shows the breakdown by block.

\begin{table}[h]
  \renewcommand{\arraystretch}{1.2}
  \caption{Average Power by Block — TT Corner}
  \label{tab:power}
  \centering
  \begin{tabular}{lccc}
    \toprule
    Block & Low & Mid & High \\
                   & (2.1\,MHz)   & (61.4\,MHz)  & (121.7\,MHz)  \\
    \midrule
...
The DAC core and load consume nearly constant power ($\approx$30\,mW) across
all input frequencies, as expected for an analog current-steering circuit.
The bias generator draws only 0.18\,mW.
The retimer is the dominant frequency-dependent power block: its dynamic
CMOS power scales with switching activity, rising from 20.9\,mW at 2.1\,MHz
to 30.6\,mW at 121.7\,MHz.
Total power ranges from 51.0\,mW to 60.8\,mW across the Nyquist band.
...
FF has faster transition and stronger switching transients, which can worsen dynamic distortion. 
...
for all corners, with the tightest margin in the FF corner at mid-frequency (48.2\,dB).
...
The output impedance meets specification across all 256 input codes:
the differential Rout is 98.76–99.84\,$\Omega$ ($<$1.1\% of the 100\,$\Omega$ target)
and the CM Rout is 24.69–24.96\,$\Omega$ ($<$1.3\% of the 25\,$\Omega$ target),
confirming the 50\,$\Omega$ load-resistor network correctly sets both impedances.
Total power ranges from 51.0\,mW (at 2.1\,MHz) to 60.8\,mW (at 121.7\,MHz),
with the retimer as the dominant frequency-dependent consumer.
